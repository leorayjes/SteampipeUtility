#!/usr/bin/env python3
"""
generate_thrifty_report.py

Generates a self-contained, filterable HTML report from one or more
Powerpipe AWS Thrifty JSON benchmark result files.

Single-benchmark usage:
    python generate_thrifty_report.py <path-to-result.json> [--output report.html]

All-Checks usage (directory of JSON files):
    python generate_thrifty_report.py <path-to-results-dir/> [--output report.html]
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate a styled HTML report from AWS Thrifty benchmark JSON output. "
            "Pass a single JSON file or a directory containing multiple JSON files."
        )
    )
    parser.add_argument(
        "input",
        help="Path to a benchmark result JSON file, or a directory containing JSON files.",
    )
    parser.add_argument(
        "--output", "-o",
        default=None,
        help="Output HTML file path. Defaults to reports/aws_thrifty.html.",
    )
...
def _walk(node: dict, service: str, category: str, controls: list) -> None:
    ...
    for ctrl in (node.get("controls") or []):
        ...
        # Deduplicate result rows by (resource, status, region, account_id).
...
def extract_controls_from_data(data: dict) -> list[dict]:
    """
    Flatten all controls from a benchmark JSON into a single list.

    Handles two formats:
    - Single-benchmark: data["groups"] has one entry (the service benchmark group).
      Category is derived from that group's title (e.g. 'EBS Checks').
    - Aggregated (All Checks): data["title"] == 'AWS Thrifty — All Checks' and
      data["groups"] has one entry per service benchmark. Each sub-group's title
      is used as the category independently.
    """
    controls: list[dict] = []
    ...
    # Aggregated format: root title signals multi-service, each group is a service.
    # Single-benchmark format: exactly one group whose title is the service name.
    # Both are handled identically — walk each top-level group with its own category.
    for group in sub_groups:
        category = group.get("title", "Unknown")
        service  = (group.get("tags") or {}).get("service", category)
        _walk(group, service=service, category=category, controls=controls)

    return controls


def load_json_files(input_path: Path) -> tuple[list[dict], str]:
    """
    Load one or more JSON files.  Returns (all_controls, account_id).

    - If input_path is a file, load it directly (handles both single-benchmark
      and pre-aggregated multi-service JSON).
    - If input_path is a directory, glob for *.json files (excluding any
      aggregated file) and merge all controls. This path is kept for
      backward-compatibility but the shell script now produces an aggregated
      JSON and passes that directly.

    account_id is inferred from the first numeric segment of the filename stem.
    """
    if input_path.is_file():
        ...
        if not json_files:
            LOGGER.error("No JSON files found in directory: %s", input_path)
            sys.exit(1)
        LOGGER.info("Found %d JSON file(s) in %s", len(json_files), input_path)
    else:
    ...
    for jf in json_files:
        LOGGER.info("Reading %s...", jf.name)
        try:
            with open(jf, encoding="utf-8") as f:
                data = json.load(f)
        except Exception as exc:
            LOGGER.warning("Skipping %s — could not parse JSON: %s", jf.name, exc)
            continue

        # Infer account ID from filename on the first parseable file.
        if account_id == "unknown":
            stem = jf.stem
            segment = stem.split("_")[0]
            if segment.isdigit():
                account_id = segment

        controls = extract_controls_from_data(data)
        all_controls.extend(controls)
        LOGGER.info("  → %d controls extracted", len(controls))

    return all_controls, account_id
...
<title>{title} — Thrifty Report</title>
<style>
...
<details id="insights-details" open>
  ...
  <div class="insights-body">
    <div class="charts-grid" id="charts-grid">
      ...
      <!-- Chart 4: Alarms by Service Category -->
      <div class="chart-card">
        <div class="chart-title">Alarms by Service Category</div>
...
<footer>
  <span>AWS Thrifty — Cost Optimisation Report</span>
...
function renderCharts(data) {{
  ...
  // Chart 3: gauge (ok / total results)
...
def generate_html(controls: list[dict], account_id: str, title: str) -> str:
    ...
    # Preserve category order as encountered (i.e. benchmark run order), deduped.
    seen: set = set()
...
def main() -> None:
    ...
    controls, account_id = load_json_files(input_path)

    if not controls:
        LOGGER.error("No controls extracted from input — aborting.")
        sys.exit(1)
    ...
    if input_path.is_dir():
        ...
        # Read the root title from the JSON. The aggregated file has
        # title = "AWS Thrifty — All Checks"; a per-service file has
        # the benchmark title (e.g. "EBS Checks") at root level.
        try:
            with open(input_path, encoding="utf-8") as f:
                raw = json.load(f)
            root_title = raw.get("title", "")
            title = root_title if root_title else "AWS Thrifty"
        except Exception:
            title = "AWS Thrifty"
    ...
    output_path = (
        ...
        if args.output
        else Path(__file__).parent / "reports" / "aws_thrifty.html"
    )

    LOGGER.info("Generating report: %s (%d controls)...", title, len(controls))
    html = generate_html(controls, account_id, title)
